...
const EmptyState: React.FC<EmptyStateProps> = ({ onChatOpen, hasInteracted }) => {
  return (
    <div className="bg-white p-8 flex flex-col items-center justify-center text-center h-[calc(100vh-8rem)]">
      <div className="mb-8">
        <div className="w-16 h-16 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center shadow-sm">
      ...
      <p className="text-gray-600 max-w-xl mb-2 text-base font-light leading-relaxed">
        Searching through endless tabs across multiple stores is exhausting. 
        <span className="font-medium text-black"> ADHIKARIAI</span> simplifies fashion discovery — matching the best outfits to your destination, occasion, and weather — so you look your best, effortlessly.
      </p>

      <div className="bg-gray-50 rounded-xl p-6 border border-gray-100 w-full max-w-sm shadow-sm">
        <p className="text-gray-800 font-light mb-3">Try something like:</p>
        <div className="space-y-2 text-gray-600 font-3xl mb-6">
        ...
        <p className="text-gray-800 font-light mb-3">Curated from leading brands</p>
        ...
        {!hasInteracted && (
          <button
            onClick={onChatOpen}
            className="mt-6 w-full flex items-center justify-center gap-2 p-3 bg-white rounded-lg border border-gray-200 hover:shadow transition-all text-gray-700 font-light"
          >
